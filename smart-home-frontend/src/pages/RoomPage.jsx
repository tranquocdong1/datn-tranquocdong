...
import {
  Thermometer, Droplets, Users, Flame,
  Wind, Lightbulb, BedDouble, Bell, BellOff,
  Sun, Moon, Power, RotateCcw, Zap,
...
/* ─── Sensor metric card ─── */
const SensorCard = ({ title, value, sub, icon, accentColor, bgColor, borderColor }) => (
  <div style={{
    flex: 1,
    minWidth: 140,
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <span style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
        {title}
      </span>
      <div style={{
    ...
    <div style={{ fontSize: 28, fontWeight: 300, color: 'var(--text-heading)', lineHeight: 1 }}>
      {value}
    </div>
...
/* ─── Device control card ─── */
const DeviceCard = ({ title, sub, icon, status, onCmd, accentColor, loading }) => {
  const isOn = status === '1';

  const cmdButtons = [
    { cmd: 'ON',   label: 'Bật',  icon: <Power size={12} strokeWidth={1.5} /> },
    { cmd: 'OFF',  label: 'Tắt',  icon: <BellOff size={12} strokeWidth={1.5} /> },
    { cmd: 'AUTO', label: 'Tự động', icon: <RotateCcw size={12} strokeWidth={1.5} /> },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2xl)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-hint)', marginTop: 3 }}>{now}</p>
        </div>
        <span style={{
          display: 'inline-flex', alignItems: 'center', gap: 6,
          fontSize: 11, fontWeight: 500, borderRadius: 999, padding: '5px 14px',
          ...
          <span style={{
            ...
            background: gasAlert ? 'var(--danger-text)' : 'var(--success-text)',
          }} />
      ...
      {/* ── Sensor cards ── */}
      <section>
        <SectionHeader title="Cảm biến môi trường" sub="Cập nhật theo thời gian thực" />
        <div style={{ display: 'flex', gap: 'var(--space-md)', flexWrap: 'wrap' }}>
          <SensorCard
            ...
            accentColor={temperature >= 35 ? 'var(--danger-text)' : 'var(--accent-primary)'}
            bgColor={temperature >= 35 ? 'var(--danger-bg)' : 'var(--accent-pale)'}
            borderColor={temperature >= 35 ? '#F09595' : 'var(--border-default)'}
          />
          <SensorCard
            title="Số người"
            value={people}
          ...
          <SensorCard
            ...
            accentColor={gasAlert ? 'var(--danger-text)' : 'var(--success-text)'}
      ...
      <div style={{ display: 'flex', gap: 'var(--space-xl)', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1.6, minWidth: 300, display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
          <SectionHeader title="Điều khiển thiết bị" sub="Bật / Tắt / Tự động" />
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 300, color: 'var(--text-heading)' }}>
